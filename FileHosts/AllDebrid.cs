...
namespace DirectPackageInstaller.FileHosts
{
    class AllDebrid : FileHostBase
    {
        ...
        public override DownloadInfo GetDownloadInfo(string URL)
        {
            ...
            var Response = DownloadString(string.Format(URLMask, Program.Config.AllDebridApiKey, HttpUtility.UrlEncode(URL)));
        ...
        {
            if (Program.Config.AllDebridApiKey.ToLowerInvariant() == "null" || string.IsNullOrEmpty(Program.Config.AllDebridApiKey))
                return false;

            if (Info == null)
            {
                var Status = DownloadString("https://api.alldebrid.com/v4/user/hosts?agent=DirectPackageInstaller&apikey=" + Program.Config.AllDebridApiKey);
